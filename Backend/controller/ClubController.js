...
const clubAdd = async (req, res) => {
    try {
        ...
        const { name } = req.body
        const data = await clubModel.create({ name, avtar })
        if (data) {
...
const clubdelete = async(req,res)=>{
    try {
        const clubId = req.params.id; // Get the club ID from the request params
    
        // Use a function from your MongoDB driver to delete the club from the database
        // For example, using Mongoose:
        const data = await clubModel.findByIdAndDelete(clubId);
        if (data) {
...
module.exports = { clubAdd, home, clubshow ,clubdelete}